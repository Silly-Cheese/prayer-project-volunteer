...
import { getFirestore, collection, getDocs, query, orderBy, doc, getDoc, updateDoc, deleteDoc, serverTimestamp } from "https://www.gstatic.com/firebasejs/12.12.1/firebase-firestore.js";
...
window.addEventListener('volunteer-admin-refresh',()=>refreshCommandCenter());

document.addEventListener('click',async event=>{
  const setting=event.target.closest('[data-setting-placeholder]');
  if(setting) await openNotice('Settings Module Prepared','This setting is now wired through the settings panel. Use the live controls in the Settings section.');
...
async function load(key,name,ordered=true){try{const snap=await getDocs(ordered?query(collection(db,name),orderBy('createdAt','desc')):collection(db,name));cache[key]=[];snap.forEach(d=>cache[key].push({id:d.id,...d.data()}));}catch(e){console.warn('Could not load '+name,e);cache[key]=[];}}
...
async function handleQuickAction(data){try{if(data.quickAction==='status'){const ok=await openConfirm('Update Record Status',`Set this record to ${data.status}?`,{confirmText:'Update'});if(!ok)return;await updateDoc(doc(db,data.col,data.id),{status:data.status,updatedAt:serverTimestamp()});await refreshCommandCenter();window.dispatchEvent(new CustomEvent('volunteer-admin-refresh-needed'));return;}if(data.quickAction==='delete'){const ok=await openConfirm(`Delete ${data.label||'record'}`,'This record will be permanently deleted.',{confirmText:'Delete',danger:true});if(!ok)return;await deleteDoc(doc(db,data.col,data.id));const deleted=!(await getDoc(doc(db,data.col,data.id))).exists();await refreshCommandCenter();if(deleted) await openNotice('Deleted','The record was deleted successfully.');else await openNotice('Deletion Not Confirmed','The delete command ran, but Firestore still returned the record. Refresh and check your Firestore rules.');}}catch(error){console.error(error);await openNotice('Action Failed',error?.message||'The action could not be completed.');}}
