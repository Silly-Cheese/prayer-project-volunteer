...
const PERMISSIONS=[
  ...
  {id:'training:complete',label:'Complete Training',description:'Can complete assigned volunteer training.',group:'Training'},
...
const PRESETS={
  ...
  trainer:['activity:create','service:log','chapter:request','outreach:create','training:complete','training:trainer'],
  admin:['activity:create','service:log','chapter:request','chapter:lead','outreach:create','prayer:team','training:complete','training:trainer','admin:limited']
};
...
// Capture which volunteer is being edited so the modal can show that volunteer's exact permissions.
...
function renderDesigner(){
  ...
  return `<section class="permission-designer" id="permissionDesigner">
    <div class="permission-head">
      <div>
        <h3>Permission Access</h3>
        <p>Choose only what this volunteer should be allowed to do. Presets are optional and can be adjusted.</p>
      </div>
      <div class="permission-count" id="permissionCount">0 selected</div>
    </div>
    <div class="permission-presets">
      <button type="button" data-permission-preset="basic">Basic</button>
      <button type="button" data-permission-preset="standard">Standard</button>
      <button type="button" data-permission-preset="chapter">Chapter Lead</button>
      <button type="button" data-permission-preset="trainer">Trainer</button>
      <button type="button" data-permission-preset="admin">Limited Admin</button>
      <button type="button" data-permission-clear>Clear</button>
    </div>
    <div class="permission-groups">
      ${groups.map(group=>`<div class="permission-group"><h4>${escapeHtml(group)}</h4>${PERMISSIONS.filter(p=>p.group===group).map(permissionCard).join('')}</div>`).join('')}
    </div>
  </section>`;
}

function permissionCard(permission){
  return `<label class="permission-card">
    <input type="checkbox" value="${escapeHtml(permission.id)}" data-permission="${escapeHtml(permission.id)}">
    <span>
...
function applyPreset(name){
  setPermissions(PRESETS[name]||PRESETS.standard);
}

function setPermissions(permissionIds){
  const ids=new Set(permissionIds||[]);
  document.querySelectorAll('[data-permission]').forEach(input=>{input.checked=ids.has(input.value);});
  const select=document.getElementById('provPermissions');
  if(select){
...
function syncSelectFromCards(){
  const selected=[...document.querySelectorAll('[data-permission]:checked')].map(input=>input.value);
  const select=document.getElementById('provPermissions');
  if(select){
...
function updateCount(){
  const count=document.querySelectorAll('[data-permission]:checked').length;
  const target=document.getElementById('permissionCount');
  if(target)target.textContent=count===1?'1 selected':`${count} selected`;
}
...
function installPermissionStyles(){
  ...
  style.textContent=`
    .permission-designer{margin-top:10px;padding:18px;border:1px solid var(--line);border-radius:24px;background:rgba(216,195,165,.045)}
    .permission-head{display:flex;gap:14px;align-items:flex-start;justify-content:space-between;margin-bottom:14px}
    .permission-head h3{margin:0 0 6px;color:var(--text)}
    .permission-head p{margin:0;color:var(--soft);line-height:1.55;font-size:13px}
    .permission-count{white-space:nowrap;border:1px solid var(--line);border-radius:999px;padding:8px 11px;color:var(--warm2);font-size:12px;font-weight:900;background:rgba(0,0,0,.24)}
    .permission-presets{display:flex;gap:8px;flex-wrap:wrap;margin:12px 0 16px}
    .permission-presets button{border:1px solid var(--line);background:rgba(0,0,0,.24);color:var(--muted);border-radius:999px;padding:9px 11px;font-weight:900;cursor:pointer}
    .permission-presets button:hover{color:var(--warm2);border-color:var(--line2);background:rgba(216,195,165,.10)}
    .permission-groups{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}
    .permission-group{border:1px solid rgba(216,195,165,.12);border-radius:20px;padding:12px;background:rgba(0,0,0,.16)}
    .permission-group h4{margin:0 0 10px;color:var(--warm);font-size:12px;letter-spacing:.12em;text-transform:uppercase}
    .permission-card{display:flex!important;grid-template-columns:none!important;align-items:flex-start;gap:10px;padding:12px;border:1px solid rgba(216,195,165,.10);border-radius:16px;background:rgba(255,255,255,.025);margin:8px 0!important;cursor:pointer}
    .permission-card:hover{border-color:var(--line2);background:rgba(216,195,165,.07)}
    .permission-card input{margin-top:4px;width:18px!important;height:18px!important;accent-color:var(--warm)}
    .permission-card span{display:block}.permission-card strong{display:block;color:var(--text);font-size:14px}.permission-card small{display:block;color:var(--soft);line-height:1.45;margin-top:3px;font-size:12px}
    @media(max-width:720px){.permission-head{display:grid}.permission-count{width:max-content}.permission-groups{grid-template-columns:1fr}}
  `;
