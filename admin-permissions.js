...
const PERMISSIONS=[
  ...
  {id:'communications:post',label:'Post Announcements',description:'Can post internal volunteer announcements.',group:'Communications'},
  {id:'recognition:create',label:'Give Recognition',description:'Can create recognition records and badges.',group:'Recognition'},
...
function applyPreset(name){setPermissions(PRESETS[name]||PRESETS.standard);}function setPermissions(permissionIds){const ids=new Set(permissionIds||[]);document.querySelectorAll('[data-permission]').forEach(input=>{input.checked=ids.has(input.value);});const select=document.getElementById('provPermissions');if(select){[...select.options].forEach(option=>{option.selected=ids.has(option.value);});}updateCount();}
